...
if(loadError){this.textureResolution=oldResolution;}else{setCookie('b3d_texture_resolution',this.textureResolution);}};this.textureScrollX=parseFloat(this.getAttribute('texture-scroll-x')||'0');this.textureScrollY=parseFloat(this.getAttribute('texture-scroll-y')||'0');this.textureScrollXElement=this.getAttribute('texture-scroll-x-element')||null;this.textureScrollXScale=parseFloat(this.getAttribute('texture-scroll-x-scale')||'0.0005');this.textureScrollYElement=this.getAttribute('texture-scroll-y-element')||null;this.textureScrollYScale=parseFloat(this.getAttribute('texture-scroll-y-scale')||'0.0005');this.textureScale=parseFloat(this.getAttribute('texture-scale')||'1');this.baseRepeat=repeat;this.textures=[];this.getCompensatedRepeat=()=>{const baseScaledRepeat=this.baseRepeat/this.textureScale;const widthCompensation=1920/window.innerWidth;return baseScaledRepeat*widthCompensation;};this.updateTextureRepeats=()=>{const compensatedRepeat=this.getCompensatedRepeat();this.textures.forEach(texture=>{if(texture){texture.repeat.set(compensatedRepeat,compensatedRepeat);}});};const compensatedRepeat=this.getCompensatedRepeat();const manifestUrl=`${textureBase}/manifest.json`;let availableTextures=new Set();fetch(manifestUrl).then(response=>{if(response.ok){return response.json();}
...
return offset;};const hasElementScrolling=this.textureScrollXElement||this.textureScrollYElement;if(hasElementScrolling){let textureOffsetX=null;if(this.textureScrollXElement){const scrollElementX=document.querySelector(this.textureScrollXElement);if(scrollElementX){const scrollTopX=scrollElementX.scrollTop;textureOffsetX=-scrollTopX*this.textureScrollXScale;textureOffsetX=normalizeOffset(textureOffsetX);}}
let textureOffsetY=null;if(this.textureScrollYElement){const scrollElementY=document.querySelector(this.textureScrollYElement);if(scrollElementY){const scrollTopY=scrollElementY.scrollTop;textureOffsetY=-scrollTopY*this.textureScrollYScale;textureOffsetY=normalizeOffset(textureOffsetY);}}
